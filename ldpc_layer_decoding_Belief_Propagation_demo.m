...
% c00194582 caizhongheng 2018.04.28 add layer_decoding and modify the encoding process
% c00194582 caizhongheng 2018.05.05
...
m           = 9;
n           = k + m;
noise_scale = 2.4;% 调整噪声功率的比值，目的是让BPSK信号既出现纠前误码又不至于超出纠前门限
iter_num    = 5; % 迭代次数
dec_grp     = bit_grp;
dec_mode    = 0; % 0: atanh mode; 1: minsum mode. 
...
%% decode
llr_data_in = llr_data(:,1:dec_grp);
...
for grp_idx=1:dec_grp
    ...
    C            = zeros(size(Hb,1),3);
    C_total      = repmat(C,length(Z),1);%每组数据独立进行，所以需要先将C_total清零
    for iter_idx=1:iter_num
        ...
        for row_idx = 1:size(Hb,1)% 按Hb逐行更新
            ...
            %% 按Hb的层计算C矩阵
            if(dec_mode==0)
                C(:,1) = 2*atanh(tanh(V(:,2)/2).*tanh(V(:,3)/2));
                C(:,2) = 2*atanh(tanh(V(:,1)/2).*tanh(V(:,3)/2));
                C(:,3) = 2*atanh(tanh(V(:,1)/2).*tanh(V(:,2)/2));
            else
...
check = abs(dec_out(1:n,:) - enc_out(1:n,1:dec_grp));
sum(sum(check))
